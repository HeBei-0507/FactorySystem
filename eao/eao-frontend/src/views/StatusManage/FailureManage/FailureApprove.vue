...
import {
  approveFailureDeal,
  getFailureApprovePage,
  getFailureDealById,
  rollbackFailureApprove
...
const lab = (a, v) => a.find((i) => i.value === v)?.label || v || ''
const map = (i) => ({
  id: i.id,
...
async function load() {
  loading.value = true
  try {
    const r = await getFailureApprovePage({
    ...
    if (!e?.elMessageNotified) ElMessage.error('获取故障审核列表失败')
  } finally {
    loading.value = false
  }
}
...
async function openRow(r) {
  ...
  try {
    ...
    if (!e?.elMessageNotified) ElMessage.error('获取故障审核详情失败')
  }
}
async function approveRow() {
  ...
  saving.value = true
  try {
    await approveFailureDeal(f.failureId)
    ElMessage.success('同意成功')
    show.value = false
    await load()
  } catch (e) {
    if (!e?.elMessageNotified) ElMessage.error('故障审核同意失败')
  } finally {
    saving.value = false
  }
}
async function rollbackRow() {
  if (!f.failureId) return ElMessage.warning('故障记录ID不能为空')
  try {
    await ElMessageBox.confirm('确认回退到故障录入吗？回退后将删除处理数据。', '提示', {
      type: 'warning'
    })
    saving.value = true
    await rollbackFailureApprove(f.failureId)
...
<template>
  <div class="failure-page">
    ...
    <div class="table-panel">
      <div class="panel-title">故障审核</div>
      <el-table v-loading="loading" :data="rows" border stripe>
        ...
        <el-table-column label="状态" min-width="110">
          <template #default="{ row }">{{ lab(allOps, row.status) }}</template>
        </el-table-column>
    ...
    <el-drawer v-model="show" title="故障审核" size="66%">
        ...
        <el-row :gutter="18">
          ...
          <el-col :span="8">
            <el-form-item label="事件等级">
              <el-input :model-value="f.severityLevel" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="处理时间">
              <el-input :model-value="f.processTime" disabled />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="18">
          <el-col :span="12">
            <el-form-item label="处理说明">
              <el-input :model-value="f.processDescription" type="textarea" :rows="3" disabled />
            </el-form-item>
          </el-col>
        </el-row>
        <div class="drawer-actions">
          <el-button type="primary" :loading="saving" @click="approveRow">同意</el-button>
