...
public class MaintenanceTicketService implements IMaintenanceTicketService {
    ...
    private Result fill(MaintenanceTicket x){
        ...
        List<EquipmentPart> parts=partMapper.list(new EquipmentPart().setPartCode(x.getPartCode()).setCreatorId(currentUserId())); if(parts==null||parts.isEmpty())return Result.fail("设备部位不存在"); if(!StringUtils.hasText(x.getPartName()))x.setPartName(t(parts.get(0).getPartName()));
